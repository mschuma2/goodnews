...
class pageFlip_plugin_base
{
	...
	function pageFlip_plugin_base()
	{
		...
		$this->options = (object)array(
			'scripts_in_head' => get_option('pageflip.scripts_in_head', '1') == '1' ? true : false
		);
	}
	...
	function init()
	{
		...
		if ($this->options->scripts_in_head)
		{
	...
	function main_page()
	{
		$tOptionsUpdated = __('Options Saved.', 'pageFlip');

		if ( !empty($_POST['pageflip_options_update']) )
		{
			if ( !empty($_POST['pageflip_options']['scripts_in_head']) )
				$this->options->scripts_in_head = true;
			else
				$this->options->scripts_in_head = false;

			update_option('pageflip.scripts_in_head', $this->options->scripts_in_head ? '1' : '0');
		...
		if ($this->options_updated)
			echo "<div class='updated'><p>{$tOptionsUpdated}</p></div>\n";

		echo $this->functions->printHeader( '<a href="http://pageflipgallery.com/">'. $this->page_title .'</a> '. PAGEFLIP_VERSION );
...
;echo '		<link rel="stylesheet" type="text/css" href="'; echo WP_PLUGIN_URL.'/'.$this->plugin_dir; ;echo '/css/pageflip-admin.css" />
		<script type="text/javascript" src="'; echo $this->jsUrl.'jquery.ui.core.js'; ;echo '"></script>
		<script type="text/javascript" src="'; echo $this->jsUrl.'jquery.ui.widget.js'; ;echo '"></script>
		<script type="text/javascript" src="'; echo $this->jsUrl.'jquery.ui.mouse.js'; ;echo '"></script>
		<script type="text/javascript" src="'; echo $this->jsUrl.'jquery.ui.draggable.js'; ;echo '"></script>
		<div class="wrap">
...
';
	...
	function manage_books()
	{
		...
		if( isset($_POST['actionButton']) )
		{
			switch($_POST['action'])
			{
			 	...
			 	case 'addpage':
			 		$this->add_page($_POST['imageId'], $_POST['type'], $book);
			 		break;
			 	...
			 	case 'Assign Selected Images to Page':
			 		if( count($_POST['images']) > 0 )
			 		{
						...
						foreach ($_POST['images'] as $imageId)
			 				$this->add_page($imageId, $_POST['type'], $book);
	...
	function add_book()
	{
		...
		$_POST['bookName'] = $wpdb->escape( $_POST['bookName'] );
		$imageId[0] = $wpdb->escape( $imageId[0] );

		$sql = "INSERT INTO `{$this->table_name}` (`name`, `date`, `bgImage`) VALUES ('{$_POST['bookName']}', '".date("U")."', '{$imageId[0]}')";
		$wpdb->query($sql);

		$book_id = $wpdb->get_var( "SELECT LAST_INSERT_ID();", 0, 0 );
		...
		if ($galleryId = $_POST['galleryId'])
		{
			...
			foreach ($images as $image)
				$this->add_page($image->id, $image->type, $newBook);

			$newBook->save();
			$redirect = "?page={$this->plugin_dir}/books";
		}
		...
		if ( !$newBook->save() )
		{
			...
			echo __('Unable to save the book! Please set permissions to the books/ , images/ , sounds/ folders and it&apos;s files to &quot;777&quot;', 'pageFlip');
			return false;
		}
	...
	function add_page( $imageId, $type, &$book )
	{
		global $wpdb;
	...
	function addPageFromGallery( $galleryId, $type )
	{
		...
		if ( count($images) > 0 )
		{
			...
			foreach ($images as $img)
				$this->add_page($img['id'], $type, $book);
	   ...
	   $dir =  $this->plugin_path . $this->imagesDir . '/';
		...
		switch( strtolower( $fileExt['1'] ) )
		{
			case 'swf' :
				...
				break;
			default :
				if( $this->functions->img_resize($tmpName, $thumbName, $newSize['width'], $newSize['height']) && $functionName($tmpName, $new_filename) )
				{
					@chmod($new_filename, 0644);
				}
				else
	...
	function edit_book()
	{
		...
		if ( empty($_POST['bookName']) ) $_POST['bookName'] = __('Untitled', 'pageFlip');
		...
		foreach( $book->properties as $property )
			if( isset($_POST[$property]) && (string)$_POST[$property] !== '' || $property == 'flipSound' || $property == 'fullscreenHint' )
